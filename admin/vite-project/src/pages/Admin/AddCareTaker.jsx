...
const AddCareTaker = () => {
    ...
    const [docuImg, setDocImg] = useState(false)
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            const { data } = await axios.post(backendUrl + '/api/admin/add-careTaker', formData, { headers: { aToken } })
            if (data.success) {
            ...
            <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
                <div className='flex items-center gap-4 mb-8 text-gray-500'>
                    <label htmlFor="doc-img">
                        <img className='w-16 bg-gray-100 rounded-full cursor-pointer' src={docuImg ? URL.createObjectURL(docuImg) : assets.upload_area} alt="" />
                    </label>
                    <input onChange={(e) => setDocImg(e.target.files[0])} type="file" name="" id="doc-img" hidden />
                    <p>Upload caretaker <br /> picture</p>
                </div>
                ...
                <button type='submit' className='bg-primary px-10 py-3 mt-4 text-white rounded-full'>Add care taker</button>
